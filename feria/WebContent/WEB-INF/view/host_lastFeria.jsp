...
<script>
    ...
    function getBibeList() {
    	console.log(left);
        //마이페이지에서 호스트가 받은 술들을 보여주기 위해 ajax를 작동하는 함수를 선언
        //) 술(비베)을 ajax로 얻어오는 메서드 선언
        $.ajax({
            // 객체선언
            ...
            success:function (json) {
                ...
                //콘솔 확인 후 출력을 위한 템플릿(언더스코어) 설정
                //each문(넘어오는 배열을 반복하여 붙이는)으로 출력을 해야 함
                ...
                //total은 lengt/8(li요소의개수)를 나눠서 1을 더한 값(Math.floor는 반올림)
                ...
                $rightBtn.on("click", function () {
                    ...
                    //console.log(left);
                    //console.log(left);
                    //만약 left와 -width값이 똑같다면 첫 페이지로 이동
                    if(left == -width) {
                        left = 0;
                    }
                    $ul.css("left",left);
    ...
    $("#select_arrange").on("change", function () {
        ...
        val = $(this).val();
  		...
  		//셀렉트가 바뀔 때, 마다 left값을 0으로 만들어서 첫 비베화면으로 보여주기
  		//안 되는 이유 값이 안들어감
  		//해결 방안: 요소에 직접적으로 초기화
  		//$(".list_bibe_collect").css({"width":width, left:0});
  		left = 0;
