...
<div id="wrapContent">
    ...
    <div id="insertCena">
        <form action="/cenaInsert.jsp" method="post">
        <input type="hidden" name="userNo" value="<%=loginMember.getNo()%>">
            ...
            <fieldset>
                ...
                <div class="insert_cena_right">
                    ...
                    <div class="host_msg not">
                        ...
                        <textarea placeholder="ex) 군대 취사병 출신, 파스타만큼은 정말 자신있습니다 !"
                                  name="hostMsg" style="outline: none" id="hostMsg" title="한마디를 입력해주세요."></textarea>
                    </div><!--//.host_msg-->
                ...
                <div class="insert_cena_bottom">
                    ...
                    <div class="btn_enrollment">
                        <button disabled class="btn" type="submit" id="enrollmentBtn">등록</button>
                    </div><!--//.btn_enrollment-->
...
function cLoad() {
    $.ajax({
        ...
        success: function (json) {
            $itemList.empty();
            console.log(json);
            //console.log(json[0].cList);
            //console.log(json[0].cList[3]);
            //console.log(json[0].cList.length);
            let cnt = 0;
            for (let i = 0; i < json.length; i++) {
                // 있으면 0, 없으면 -1
                console.log(json[i].cookName.indexOf($insertTag.val()));
                if ($insertTag.val() != "") {
                    if (json[i].cookName.indexOf($insertTag.val()) != -1) {
                        const $li = $("<li>").text(json[i].cookName);
                        $li.appendTo("#itemList");
                        console.log("cookNo : "+json[i].cookNo);
                        //cookNoArr[cnt]=json[i].cookNo;
                       	//for(let j = 0; j < cookNoArr.length; j++){
                        //console.log("배열 : "+cookNoArr[j]);
                        //console.log("전체배열 : "+cookNoArr);
                       	//}
                        cnt++;
                    }
                }
...
function putTag(text){
	...
	$.ajax({
		...
		success : function (json) {
			 $("#insertTagBox").before(tmp({cook:json}));
			 cnt++;
		}
	});
...
$itemList.on("click", "li", function () {
    cLoad();    // 클릭시 input에 값 넘어가고 해당 글자랑 같은거만 보여주려고

    //$("#itemList").children().removeClass("on");
    //$(this).addClass("on");

    $insertTag.val($(this).text());
    ...
    $("#insertCenaImg").change(function () {
		...
		$.ajax({
			...
			success:function(json){
				console.log(json);
				//유저에게 미리보기 제공
				$("#cenaImg").attr("src","/upload/"+json.img);
				
				//cenaInsert.jsp에 넘어갈 사진이름을 세팅
				$("#cenaImgName").val(json.img);
			}
		});//#ajax end
    });
